...
class JobListingModel
{
    ...
    {
        $this->db->query("SELECT jl.jobListing_id, jl.job_title, jl.description, jl.published_time, e.company_name, e.email, l.location, jt.jobType FROM joblisting as jl
        INNER JOIN location as l on jl.location_id = l.location_id INNER JOIN employer as e ON jl.employer_id = e.employer_id INNER JOIN jobtype as jt on jl.jobType_id = jt.jobType_id");
        $row = $this->db->fetchSingleRow();
